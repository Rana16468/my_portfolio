        ...
        <div style={{ position: 'relative', height: 16, marginBottom: 4, minWidth: svgWidth }}>
          {monthLabels.map(({ label, col }) => (
            <span key={label} style={{
              ...
              fontSize: 10, color: '#8b949e', fontFamily: 'monospace', whiteSpace: 'nowrap',
            }}>{label}</span>
          ))}
        </div>
        {/* Grid */}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>
          ...
          <a href={repo.html_url} target="_blank" rel="noreferrer"
            style={{ color: '#58a6ff', fontWeight: 600, fontSize: 14, textDecoration: 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {repo.name}
          </a>
          {repo.private
      ...
      <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 12, fontSize: 11, color: '#8b949e' }}>
        ...
        <span style={{ color: '#58a6ff', marginLeft: 'auto' }}>
          Created {created.toLocaleDateString('en-US', { year: 'numeric', month: 'short' })}
        </span>
        {repo.updated_at && <span>Updated {fmtDate(repo.updated_at)}</span>}
      </div>
    </div>
  );
}
          ...
          <main>
            ...
            <div style={{ background: '#161b22', border: '1px solid #21262d', borderRadius: 10, padding: '14px 16px', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, flexWrap: 'wrap', gap: 6 }}>
                ...
                <span style={{ fontSize: 11, color: '#484f58', fontFamily: 'monospace' }}>simulated · auth required for real data</span>
              </div>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              {currentRepos.length > 0
                ...
                : (
                  <div style={{ background: '#161b22', border: '1px solid #21262d', borderRadius: 10, padding: 28, textAlign: 'center', color: '#8b949e', fontSize: 13 }}>
